...
const int max_jump = 316;

int pref_sum [max_jump+2][MAXN];
int dp_sum [max_jump+2][MAXN];
...
void calculate()
{
    ...
    for (int jump = 1 ; jump <= max_jump ; jump++)
    {
        ...
        {
            pref_sum[jump][pos+jump] = pref_sum[jump][pos] + arr[pos];

            dp_sum[jump][pos+jump]= dp_sum[jump][pos] + arr[pos]*(pos/jump+1);
        }
    }
    return;
}
...
void answer()
{
    ...
    while(q--)
    {
        ...
        int dp = dp_sum[d][en]-dp_sum[d][s];

        int sum = ((pref_sum[d][en]-pref_sum[d][s])*(s/d));
